            ...
            <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
              <Card className="transition duration-300 hover:-translate-y-1">
                <div className="mb-5 flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm text-cyan-200">AI planning status</p>
                    <h3 className="text-2xl font-semibold text-white">Your trip is nearly ready</h3>
                    <p className="mt-2 text-sm text-slate-400">A calm, coordinated plan for your group.</p>
                  </div>
                ...
                <div className="rounded-[24px] border border-white/10 bg-slate-950/70 p-4">
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      <p className="text-sm text-slate-400">Itinerary ready</p>
                      <p className="text-lg font-semibold text-white">Thursday to Sunday flow</p>
                    </div>
                    <div className="rounded-full bg-emerald-500/15 px-3 py-1 text-sm text-emerald-200">Ready</div>
                  </div>
                  <div className="space-y-3">
                    {mockTrip.itinerary.map((item) => (
                      <div key={`${item.day}-${item.time}`} className="flex items-start gap-3 rounded-2xl border border-white/10 bg-white/5 p-3">
              ...
              <Card className="transition duration-300 hover:-translate-y-1">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <p className="text-sm text-cyan-200">Expense summary</p>
                    <h3 className="text-2xl font-semibold text-white">Shared costs</h3>
                  </div>
            ...
            <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
              <Card className="transition duration-300 hover:-translate-y-1">
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm text-cyan-200">Group members</p>
                    <h3 className="text-2xl font-semibold text-white">People and preferences</h3>
                  </div>
                  <div className="flex -space-x-2">
                    {members.slice(0, 4).map((member, idx) => (
                      <div key={member.id} className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-slate-900 bg-gradient-to-br from-cyan-400 to-violet-500 text-sm font-semibold text-white">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {loading ? (
                    <p className="text-sm text-slate-400">Loading group members…</p>
                  ) : (
              ...
              <Card className="transition duration-300 hover:-translate-y-1">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <p className="text-sm text-cyan-200">TripBuddy AI</p>
                    <h3 className="text-2xl font-semibold text-white">Ask for planning help</h3>
                  </div>
                  <div className="rounded-full bg-cyan-500/15 p-2 text-cyan-200">
                    <Bot size={18} />
                  </div>
                </div>
                <div className="mb-4 rounded-[24px] border border-white/10 bg-slate-950/60 p-4 text-sm leading-7 text-slate-300">
