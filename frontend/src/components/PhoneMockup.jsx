      ...
      <div className="relative w-[360px]">
        {screens.map((screen, index) => (
          <motion.div
            ...
            className="absolute left-0 right-0 top-0 rounded-[40px] border border-white/80 bg-white shadow-[0_35px_90px_rgba(16,24,40,0.18)]"
          >
            <div className="h-[680px] overflow-hidden rounded-[32px] bg-gradient-to-b from-white via-[var(--bg-secondary)] to-[var(--accent-sky)] p-4">
              <div className="flex items-center justify-between rounded-[28px] bg-white/90 px-4 py-3 shadow-sm shadow-slate-200/40">
                <div>
              ...
              <div className="mt-4 h-full overflow-hidden rounded-[28px] border border-white/70 bg-[var(--bg-primary)] p-4 shadow-inner shadow-slate-200/50">
                {screen.content}
              </div>
            </div>
          </motion.div>
        ))}
        <div className="absolute -bottom-6 left-1/2 w-72 -translate-x-1/2 rounded-full bg-white/80 px-4 py-2 text-center text-xs text-[var(--text-secondary)] shadow-sm shadow-slate-200/70">
          Touch-ready preview
        </div>
      </div>
    </div>
